...
## Unix time of the last completed sync, or 0 if this session has not had one.
var last_sync := 0
...
func note() -> String:
	match state:
		State.READY:
			if last_sync == 0:
				return "your progress is on this Apple ID"
			var t := Time.get_datetime_dict_from_unix_time(last_sync)
			return "saved to your Apple ID at %02d:%02d" % [int(t["hour"]), int(t["minute"])]
		State.SYNCING:
			return status
		State.WAITING:
...
func _on_fetched(games, error) -> void:
	if error != null or games == null:
		_busy = false
		# The likeliest cause by far is a build with no iCloud entitlement, which
		# is invisible from inside the game and looks exactly like a network
		# failure. Said out loud in the device log for that reason.
		push_warning("Cloud: could not read saved games — %s" % str(error))
		_set_state(State.FAILED, "iCloud is not reachable")
		return
...
func _on_loaded(data, error) -> void:
	if error != null:
		push_warning("Cloud: a saved game would not open — %s" % str(error))
...
func push() -> void:
	...
	MultiplayerManager.local_player.save_game_data(
		Profile.to_bytes(), SAVE_NAME, _on_saved)
...
func _on_saved(_game, error) -> void:
	...
	if error != null:
		...
		push_warning("Cloud: save refused — %s" % str(error))
		_set_state(State.FAILED, "iCloud would not take the save")
		return
	_cloud_behind = false
	_settled(true)
